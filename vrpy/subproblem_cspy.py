...
class _MyREFCallback(REFCallback):
    ...
    def __init__(self, max_res, sub_G, time_windows, distribution_collection, T,
        ...
        self._original_node_type = type(
            [n for n in self._sub_G.nodes() if n not in ["Source", "Sink"]][0])
    ...
    def REF_fwd(self, cumul_res, tail, head, edge_res, partial_path,
        ...
        theta_t = self._sub_G.nodes["Sink"]["service_time"]
        # Travel times
        travel_time_ij = self._sub_G.edges[i, j]["time"]
        try:
            travel_time_jt = self._sub_G.edges[j, "Sink"]["time"]
        except KeyError:
            travel_time_jt = 0
        # Time windows
        # Lower
        a_j = self._sub_G.nodes[j]["lower"]
        a_t = self._sub_G.nodes["Sink"]["lower"]
        # Upper
        b_j = self._sub_G.nodes[j]["upper"]
        b_t = self._sub_G.nodes["Sink"]["upper"]
