...
class VehicleRoutingProblem:
        ...
        for vehicle in range(self._vehicle_types):
            ...
            if pricing_strategy == "Stops":
                for stop in range(2, self.num_stops):
                    subproblem = self._def_subproblem(
                        duals, vehicle, cspy, exact, solver, pricing_strategy, stop,
                    )
            ...
            if pricing_strategy == "PruneEdges":
                for alpha in [0.3, 0.5, 0.7, 0.9]:
                    subproblem = self._def_subproblem(
                        duals, vehicle, cspy, exact, solver, pricing_strategy, alpha,
                    )
                    self.routes, more_routes = subproblem.solve(time_limit)
                    if more_routes:
                        break
            ...
            if not more_routes or pricing_strategy == "Exact":
                subproblem = self._def_subproblem(duals, vehicle, cspy, exact, solver,)
                self.routes, more_routes = subproblem.solve(time_limit)
    ...
    def _check_consistency(self, cspy):
